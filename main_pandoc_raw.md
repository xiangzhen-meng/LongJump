...
代入式[\[eq:orbit_exact\]](#eq:orbit_exact){reference-type="eqref"
reference="eq:orbit_exact"}得：
...
$\dot\theta_2 = \dot\theta_1 + \dot\phi$。代入式[\[eq:angular_momentum_exact\]](#eq:angular_momentum_exact){reference-type="eqref"
reference="eq:angular_momentum_exact"}整理得：
...
\end{align}$$ 其中 $\dot\phi(t)$ 由运动员主动控制（例如匀速收腿）。
...
利用式[\[eq:dt1\]](#eq:dt1){reference-type="eqref"
reference="eq:dt1"}和[\[eq:dt2\]](#eq:dt2){reference-type="eqref"
reference="eq:dt2"}积分得到
$\theta_1(t),\theta_2(t)$，代入[\[eq:Dx\]](#eq:Dx){reference-type="eqref"
reference="eq:Dx"}即可得 $\Delta x(t)$。落地时刻 $t=T$ 的 $\Delta x(T)$
即为通过姿态调整获得的额外跳跃距离。
...
$t=T$ 时的数值由式[\[eq:Dx\]](#eq:Dx){reference-type="eqref"
reference="eq:Dx"} 给出（结果见第 3.2 节）。
...
正文表 [1](#tab:two_body_params){reference-type="ref"
reference="tab:two_body_params"}
中的转动惯量和收腿角速度按以下方式估算。
...
$72.3\pm13.0$ kg）在自由摆臂（JFA）和限制摆臂（JRA）两种条件下分别测量了立定跳远的运动学参数。关键实验数据汇总于表 [2](#tab:exp_data){reference-type="ref"
reference="tab:exp_data"}。
...
::: {#tab:exp_data}
  ...
  : 自由臂（JFA）与限制臂（JRA）实验数据对比。$\Delta x$
...
模型的输出为落地的脚超前质心距离
$\Delta x(t)$，其由腾空阶段的角动量守恒和收腿运动决定。取表 [1](#tab:two_body_params){reference-type="ref"
reference="tab:two_body_params"} 中的参数（对应 JRA）及收腿角速度
...
取髋关节 $O$ 为参考点，各刚体质心和肩关节的位矢： $$\begin{align}
...
\end{align}$$ 其中 $\hat{\mathbf{k}}$ 为垂直于运动平面的单位向量（$z$
轴正方向）。
...
相对位置
$\boldsymbol{\rho}_i \equiv \mathbf{r}_i - \mathbf{r}_G$（$i=T,A,L$）。由式[\[eq:rT\]](#eq:rT){reference-type="eqref"
reference="eq:rT"}--[\[eq:rG_three\]](#eq:rG_three){reference-type="eqref"
reference="eq:rG_three"}： $$\begin{align}
...
合并同类项，最终将式[\[eq:LG_three\]](#eq:LG_three){reference-type="eqref"
reference="eq:LG_three"}化为： $$\begin{equation}
\boxed{A_T(\boldsymbol{\theta})\,\omega_T \;+\; A_A(\boldsymbol{\theta})\,\omega_A \;+\; A_L(\boldsymbol{\theta})\,\omega_L \;=\; 0}, \label{eq:three_body_am}
\end{equation}$$ 其中系数函数 $A_i(\boldsymbol{\theta})$
仅依赖当前姿态角度： $$\begin{align}
...
为便于与二体模型对比，写出相对于躯干的相对角度： $$\begin{align}
...
\end{align}$$ 其中 $\dot\varphi_{LT}$ 即为前文二体模型中的\"收腿角速度\"
$\dot\phi$，$\dot\varphi_{TA}$ 为\"摆臂角速度\"。

将
$\omega_A = \omega_T + \dot\varphi_{TA}$、$\omega_L = \omega_T + \dot\varphi_{LT}$
代入式[\[eq:three_body_am\]](#eq:three_body_am){reference-type="eqref"
reference="eq:three_body_am"}，整理得： $$\begin{equation}
\boxed{\omega_T = -\frac{A_A\,\dot\varphi_{TA} \;+\; A_L\,\dot\varphi_{LT}}{A_T + A_A + A_L}}. \label{eq:omega_T_relative}
\end{equation}$$

当手臂相对于躯干固定（$\dot\varphi_{TA} = 0$），即JRA情形，式[\[eq:omega_T_relative\]](#eq:omega_T_relative){reference-type="eqref"
reference="eq:omega_T_relative"}简化为
...
由式[\[eq:rG_three\]](#eq:rG_three){reference-type="eqref"
reference="eq:rG_three"}，总质心水平坐标： $$\begin{equation}
x_G = x_O + \frac{m_T d_T \cos\theta_T + m_A (L_T \cos\theta_T + d_A \cos\theta_A) + m_L d_L \cos\theta_L}{M}.
\end{equation}$$

因此落地前伸量（脚超前质心的水平距离）： $$\begin{equation}
...
$\varphi_{TA}(0)=180^\circ$（臂与躯干平行，指向髋部）作为与二体模型的对照。数值积分得到表 [3](#tab:three_body_results){reference-type="ref"
reference="tab:three_body_results"}。
...
::: {#tab:three_body_results}
  ...
  : 三刚体模型典型算例结果对比。三体JRA(145$^\circ$)为臂贴于身前，三体JRA(180$^\circ$)为臂与躯干平行指向髋部
:::
...
的变化，结果如图 [1](#fig:sweep){reference-type="ref"
reference="fig:sweep"} 所示。

<figure id="fig:sweep" data-latex-placement="h">
<embed src="fig_phiTA_sweep.pdf" style="width:85.0%" />
<figcaption>三体 JRA 中手臂静态固定姿态 <span
class="math inline"><em>φ</em><sub><em>T</em><em>A</em></sub></span>
对落地前伸量 <span class="math inline"><em>Δ</em><em>x</em></span>
的影响。 水平虚线分别标注实验 JFA（<span
class="math inline"><em>Δ</em><em>x</em> = 0.31</span> m）和 JRA（<span
class="math inline"><em>Δ</em><em>x</em> = 0.29</span> m）。 曲线在
<span
class="math inline"><em>φ</em><sub><em>T</em><em>A</em></sub> = 180<sup>∘</sup></span>（臂平行躯干）处取极小值
<span class="math inline">0.294</span> m， 在 <span
class="math inline"><em>φ</em><sub><em>T</em><em>A</em></sub> = 0<sup>∘</sup></span>
或 <span
class="math inline">360<sup>∘</sup></span>（臂与躯干同向）附近取极大值
<span class="math inline"> ∼ 0.383</span> m。</figcaption>
</figure>
...
未必都需要归因于腾空阶段手臂的主动摆动。图 [1](#fig:sweep){reference-type="ref"
reference="fig:sweep"} 显示，
